...
Standard JWT flow is used. When you log in, you get a token that has your user ID and role in it. On the backend there are two middleware functions — one called `protect` that verifies the token, and one called `admin` that checks the role before giving access to admin routes. On the frontend, AuthContext handles the login state and ProtectedRoute redirects customers away from admin pages, ensuring strict role separation.
...
- No email or SMS confirmations are sent upon booking

## Areas for improvement with additional time

- Time slots should ideally be configurable from the admin side rather than hardcoded
- Data fetching on the frontend uses basic useEffect — implementing React Query would provide better caching and state management
- No automated tests yet, would add Jest and Supertest for comprehensive API testing
